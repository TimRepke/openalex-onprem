...
def request_meta_cache(url: str, meta_key: str, buffer: list[WorkOut], wrapper: str | None = None):
    ...
    try:
        ...
        info = res.json()
        logging.info(
            f'Hits: {info["n_hits"]}, updates: {info["n_updated"]}, '
...
@app.command()
def update_solr(
        api_key: Annotated[str, typer.Option(help='OpenAlex premium API key')],
    ...
    while cursor is not None:
        ...
        with rate_limit(min_time_ms=100) as t:
            ...
            logging.info(
                f'Retrieved {n_works:,} / {page['meta']['count']:,} '
...
def request_abstracts(
        ...
        created_since: Annotated[str, typer.Option(callback=date_check, help='Get works created on or after')],
        created_until: Annotated[str | None, typer.Option(
            callback=date_check,
    ...
    with httpx.Client() as client:
        ...
        while True:
            ...
            logging.debug(
                f'Done with page {page} in {timedelta(seconds=time() - t1)}h; '
...
def backfill_abstracts(
        ...
        created_since: Annotated[str, typer.Option(
            callback=date_check,
            help='Get works created on or after',
        )],
        batch_size: int = 200,
