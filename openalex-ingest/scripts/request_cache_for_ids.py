    ...
    with httpx.Client() as client:
        for bi, batch in enumerate(batched(openalex_ids, BATCH_SIZE)):
            ...
            if len(remaining) > 0:
                ...
                cache_response = ScopusWrapper.run(
                    db_engine=db_engine_cache, references=[Reference(openalex_id=doc.openalex_id, doi=doc.doi) for doc in remaining], auth_key=auth_key
                )
            else:
