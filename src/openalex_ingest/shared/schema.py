...
from .models import SourcePriority, OnConflict, enum_validator
...
class SourcesJSONB(TypeDecorator):
    """Custom JSONB type that handles SourcePriority Enums."""

    impl = JSONB

    def process_bind_param(self, value: list[tuple[APIEnum, SourcePriority]] | None, dialect: Any) -> list[tuple[str, int]]:
        """This function handles the Python -> DB conversion"""
        if value is None or len(value) == 0:
            return None

        return [(source[0].value, source[1].value) for source in value]

    def process_result_value(self, value: list[tuple[str, int]] | None, dialect: Any) -> list[tuple[APIEnum, SourcePriority]] | None:
        """This function handles the DB -> Python conversion"""
        if value is None:
            return value
        return [(APIEnum(source[0]), SourcePriority(source[1])) for source in value]
...
class Queue(SQLModel, table=True):
    ...
    sources: list[tuple[APIEnum, SourcePriority]] | None = Field(sa_column=Column(SourcesJSONB(none_as_null=True)), default=None)
...
class QueueRequests(BaseModel):  # FIXME: class QueueRequests(Queue, table=False) throws type error
    ...
    on_conflict: Annotated[OnConflict, enum_validator(OnConflict)] = OnConflict.DO_NOTHING

    time_created: datetime
    source: APIEnum
    #  end hack

    priority: Annotated[SourcePriority, enum_validator(SourcePriority)]
    num_has_request: int
