...
class OpenAlexToCitationIndex:
    ...
    def _check_schema(self, df: pd.DataFrame, schema_path: str) -> pd.DataFrame | None:
    ...
    def _load_openalex_sources(self) -> None:
        ...
        oa = self._check_schema(oa, 'schemas/openalex_sources.yml')
        if oa is None:
            self.open_alex = None
            return
        self.open_alex = oa
    ...
    def _read_and_match_scopus_files(self, base_dir: Path) -> None:
        for fn in base_dir.rglob('*'):
            ...
            if citation_index is None:
                ...
                continue
            df = pd.read_excel(fn)
            df = self._check_schema(df, 'schemas/scopus.yml')
            if df is None:
            ...
            df = self._check_schema(df, 'schemas/scopus_preprocessed.yml')
            if df is None:
    ...
    def _read_and_match_web_of_science_files(self, base_dir: Path) -> None:
        for fn in base_dir.rglob('*'):
            ...
            if citation_index is None:
                ...
                continue
            df = pd.read_csv(fn)
            df = self._check_schema(df, 'schemas/webofsci.yml')
            if df is None:
                logger.info(f'Skipping file {fn} because schema validation failed.')
                self.index_names.pop(fn)
                continue
